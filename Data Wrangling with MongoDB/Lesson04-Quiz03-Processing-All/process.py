...
def process_file(f):
    ...
    with open("{}/{}".format(datadir, f), "r") as html:
        ...
        t = soup.find(id="DataGrid1").find_all("tr")
        for i in range(1, len(t)):
            info["courier"], info["airport"] = f[:6].split("-")
            d = t[i].find_all("td")
            if d[1].get_text() != "TOTAL":
                info["year"] = int(d[0].get_text())
                info["month"] = int(d[1].get_text())
                flights = {}
                flights["domestic"] = int(d[2].get_text().replace(',', ''))
                flights["international"] = int(d[3].get_text().replace(',', ''))
